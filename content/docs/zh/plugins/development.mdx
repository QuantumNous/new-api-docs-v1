...
此示例通过通用接口 `POST /v1/tasks/demo-task` 提交，JSON body 包含 `model: "demo-model"` 和 `prompt`。它没有声明原生路由、Image/Video/Responses 协议或产物钩子，这些能力需要在实际插件中按需实现。
...
- **宿主协议**：声明 `meta.protocols`，为 Image、Video 或 Responses 实现协议钩子。
- **产物**：成对实现 `listArtifacts` 和 `buildContentRequest`。
- **提交即完成或上游 SSE**：按能力声明实现即时终态、SSE 快照或增量事件钩子。
- **网关互联**：实现厂商与 New API 上游路径、提交解析和查询后，再声明 `upstreams: ["new_api"]`；覆盖 `ctx.upstream.kind` 的两种取值和缺失字段回退。
