...
## Connecting to another New API gateway

When the upstream is also New API, select **New API (Type 60)**, enter the upstream gateway URL and its API key, and bind plugins supporting the `new_api` upstream kind. Both gateways must have the same plugin installed and enabled, with versions supporting the required contract. A plugin cannot bind to this channel unless `meta.upstreams` declares `new_api`.

Type 60 supports multiple plugins through `setting.task_extend_plugin_keys` and also accepts `setting.task_plugin_key`. If several bound plugins support a model, the first eligible candidate in ascending plugin-key order executes. The host sends the upstream gateway key as Bearer credentials; the plugin constructs its native paths according to `ctx.upstream.kind`. Vendor API keys or OAuth credentials are not upstream gateway keys.
...
Usage profile selection first checks `ctx.upstreamModel`, falls back to `ctx.model` if the upstream name has no profile, and uses the plugin default schema only if neither matches. Mapping a model to a vendor endpoint ID therefore preserves its client-facing usage profile.
