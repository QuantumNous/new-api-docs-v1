...
At runtime, the upstream model profile takes precedence. If the mapped name has no profile, selection falls back to the client-facing model, then to the defaults. This also applies to background and batch polling.
...
## Image protocol usage

`openai_image` follows the ordinary task billing contract: `extractUsage` reports the requested count. Immediate results report actual counts through `extractUsageOnSubmit` for legacy pricing; task expressions use `extractUsageOnComplete` for immediate and polled results. When facts contain `image_count`, the consume log records the billed count in `other.image_count`.

Count actual images, not entries containing only text, prompts, or empty payloads. A timeout while waiting for images or a client disconnect does not cancel a submitted upstream task; it still settles on background completion. A missing create response does not mean the task was free and is not a reason to immediately resubmit it.
