...
export const meta = {
  ...
  usageSchema: {
    requests: {
      type: 'number',
      unit: 'count',
      description: { en: 'Generation request unit price', zh: '生成请求单价' },
    },
  },
...
This example submits via the generic interface `POST /v1/tasks/demo-task`, with a JSON body containing `model: "demo-model"` and `prompt`. It does not declare native routes, Image/Video/Responses protocols, or artifact hooks, which need to be implemented as needed in actual plugins.
...
- **Host Protocol**: Declare `meta.protocols` and implement protocol hooks for Image, Video, or Responses.
...
- **Gateway connections**: Implement vendor and New API paths, submission parsing, and querying before declaring `upstreams: ["new_api"]`; cover both `ctx.upstream.kind` values and the missing-field fallback.
