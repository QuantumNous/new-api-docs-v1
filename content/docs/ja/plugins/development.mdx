...
この例は、汎用インターフェース `POST /v1/tasks/demo-task` を介して送信され、JSONボディには `model: "demo-model"` と `prompt` が含まれます。ネイティブルート、Image/Video/Responses プロトコル、または成果物フックは宣言されていません。これらの機能は、実際のプラグインで必要に応じて実装されます。
...
- **ホストプロトコル**：`meta.protocols` を宣言し、Image、Video、Responses のプロトコルフックを実装します。
- **成果物**：`listArtifacts` と `buildContentRequest` をペアで実装します。
- **即時完了またはアップストリームSSE**：機能宣言に従って、即時最終状態、SSEスナップショット、または増分イベントフックを実装します。
- **ゲートウェイ間接続**：ベンダーと New API のパス、送信結果解析、照会を実装してから `upstreams: ["new_api"]` を宣言します。`ctx.upstream.kind` の両方の値と、フィールドがない場合のフォールバックを検証します。
